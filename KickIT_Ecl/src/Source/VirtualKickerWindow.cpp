...
//VirtualKickerWindow::VirtualKickerWindow() {
	...
	bottomRight = new QPoint(WINDOW_SIZE_X - TABLE_MARGIN,
			WINDOW_SIZE_Y - TABLE_MARGIN);
	keeperBar = new QLine(topLeft->x() + 20, topLeft->y() - 10,
			topLeft->x() + 20, bottomRight->y() + 10);
...
void VirtualKickerWindow::paintEvent(QPaintEvent *event) {
	...
	painter.drawRect(topLeft->x(), topLeft->y(),
			WINDOW_SIZE_X - 2 * TABLE_MARGIN, WINDOW_SIZE_Y - 2 * TABLE_MARGIN);
