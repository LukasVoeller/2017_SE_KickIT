...
protected:
	float* calculateRowPositions(float tableHeight, BallStatus* b, bool keeper,
			...
			Vec2* defensePositionalVector = 0, Vec2* defenseDirectionalVector =
					0, Vec2* midfieldPositionalVector = 0,
			Vec2* midfieldDirectionalVector = 0, Vec2* offensePositionalVector =
					0, Vec2* offenseDirectionalVector = 0) {
		...
		if (keeper) {
			...
			if (b->movement.cross(*keeperDirectionalVector) != 0) {
				float factor = ((*keeperPositionalVector - b->movement).cross(
						*keeperDirectionalVector))
						/ b->movement.cross(*keeperDirectionalVector);
