...
VirtualKickerWindow::VirtualKickerWindow():tc(NULL) {
    ...
    keeperBar = new QLine(topLeft->x()+goalToKeeper, topLeft->y()-30, topLeft->x()+goalToKeeper, topLeft->y()+tableDimensionY+30);
    keeper = new QPoint(keeperBar->x1(), keeperBar->y1() + keeperBar->dy()/2);
    
    defenseBar = new QLine(topLeft->x()+goalToDefense, topLeft->y()-30, topLeft->x()+goalToDefense, topLeft->y()+tableDimensionY+30);
...
void VirtualKickerWindow::paintEvent(QPaintEvent *event) {
    ...
    painter.drawRect(topLeft->x()-30, topLeft->y()+tableDimensionY/2-goalSize/2, 30, goalSize);	//Tor
    painter.drawRect(topLeft->x(), topLeft->y(), tableDimensionX, tableDimensionY);				//Tisch
