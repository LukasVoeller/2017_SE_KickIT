...
void motor_by_hand() {
	...
	cout << "Ziel Position eingeben und mit enter bestaetigen" << endl;
		...
		frame_init(0x201, 0x8, 0x3F, 0x00, nibble, 0x09, pos1,
				pos2, 0xBB, 0x08); // RXPDO 1
			...
			while (camera.IsGrabbing()) {
				...
				if (ptrGrabResult->GrabSucceeded()) {
					...
					if (countNonZero(imgThresholded) > 0) {
						...
						circle(cv_img, Ballcenter, 3, Scalar(0, 255, 0), -1, 8,
								0);
						// draw the circle outline
						circle(cv_img, Ballcenter, 10, Scalar(0, 0, 255), 3, 8,
								0);
						lasty = Ballcenter.y;
						lastx = Ballcenter.x;
					}
				...
				auto t2 = std::chrono::high_resolution_clock::now();
							...
							if(countNonZero(imgThresholded) > 0) {
								...
								Ballcenter.y = Ballcenter.y /2;
								 ...
								 if(Ballcenter.x == lastx || Ballcenter.y == lasty) TorwartSoll.y = Ballcenter.y; // Wenn der Ball Steht
					 				...
					 				if(TorwartSoll.y < TorwartMin.y || TorwartSoll.y > TorwartMax.y) TorwartSoll.y = lasty; //
					 				 	 }
					 			if(abs(TorwartSoll.y-lasty) < 2){TorwartSoll.y = lasty;} // Kleine Hysterese, dass der Motor nicht bei jedem neuen angesteuerten Pixel verfährt
							dummy = 0;
							}
							else{
								dummy++;
